...
PRIVATE_CSV = Path("Hackathon Ensimag \u2014 Flash Trading Challenge - Palier 2.csv")
PRIVATE_EXPECTED = Path("tests/fixtures_private/level2/palier_expected.json")


@pytest.mark.parametrize("fixture", PUBLIC_FIXTURES, ids=lambda p: p.stem)
def test_example_team_level2_public(fixture):
...
def test_example_team_level2_private():
    passed, message = validate_level_csv("example_team", "level2", PRIVATE_CSV, PRIVATE_EXPECTED)
    assert passed, message
